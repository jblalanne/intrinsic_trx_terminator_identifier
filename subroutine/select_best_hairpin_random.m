function [ind_best_hairpin,bool_cut] = select_best_hairpin_random(...
    MFE,n_hairpins,n_bp,loop_size,distance_stem_3pr,fraction_in_stem,cut)

% number of sequences, and lengths folded per sequence
n_seq = size(MFE,1);
l_per_seq = size(MFE,2);

% ranking order for properties: 1 for high, 0 for low.
rank_order = [1 1 1 0];
...
for i = 1:n_seq
    ...
    if isempty(inds)
        ...
        prop(3,:) = squeeze(distance_stem_3pr(i,inds));
        prop(4,:) = squeeze(fraction_in_stem(i,inds));
